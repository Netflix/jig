...
A consumer POM is generated from each module descriptor. Non-system `requires` directives become Maven dependencies, and `requires static` becomes an optional compile dependency. Dependencies outside the artifact directory are mapped back to their original Maven coordinates through the same module-location contract used for resolution.
